...
```{r}
m19 <- ggplot(data = youtube, aes(fill = ratings_disabled, y = comments_disabled)) +
  geom_bar(position = 'fill')

m20 <- ggplot(data = youtube, aes(x = num_caps, y = comments_disabled)) +
  geom_boxplot()

m21 <- ggplot(data = youtube, aes(fill = num_exc, y = comments_disabled)) +
  geom_bar(position = 'fill')

m22 <- ggplot(data = youtube, aes(fill = num_period, y = comments_disabled)) +
  geom_bar(position = 'fill')

m23 <- ggplot(data = youtube, aes(fill = num_dollar, y = comments_disabled)) +
  geom_bar(position = 'fill')

m24 <- ggplot(data = youtube, aes(x = desc_length, y = comments_disabled)) +
  geom_boxplot()
...
`subscriberCount`, `category`, `comments_disabled`, `ratings_disabled`, `num_caps`, `num_exc`, `num_period`, `num_dollar`, `desc_length`, `hour_published`, `weekday_published`
...
```{r}
full_model_factor <- lm(log_views ~ subscriberCount + category + comments_disabled + ratings_disabled + num_caps + num_exc + num_period + num_dollar + desc_length + hour_published + weekday_published + day_published, data = youtube)

full_model_int <- lm(log_views ~ subscriberCount + category + comments_disabled + ratings_disabled + num_caps + num_exc + num_period + num_dollar + desc_length + hour_published + weekday_published + day_published, data = youtube_int)

glance(full_model_factor)
glance(full_model_int)
```

Looking at the AIC and BIC of our 2 models, it looks like our model with `full_model_int` is considered better. This isn't super surprising as they have fairly close values of $R^2$ but `full_model_factor` has 91 degrees of freedom compared to `full_model_int` with only 80. Seeing as AIC and BIC penalizes more complex models, this is expected. Since the difference in $R^2$ is still fairly small, we will be building off of `full_model_int`.

```{r}
youtube <- youtube_int
```

Consider the model output of `full_model_int`.
