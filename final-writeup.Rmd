...
```{r echo=FALSE}
youtube <- youtube %>%
  mutate(hour_published = as.factor(hour_published))
ggplot(data = youtube, aes(x = weekday_published, y =log_views)) +
  geom_boxplot() +
  labs(x = 'Weekday Published', y = 'Log Views', title = 'Weekday Published vs Log View Count')
```
